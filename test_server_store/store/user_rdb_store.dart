...
class UserRedisStore extends IRedisStore {
  ...
  static Future add(User model) {
    if (model is! User) throw new IStoreException(20022, [model.runtimeType, _modelName]);
  ...
  static Future set(User model) {
    if (model is! User) throw new IStoreException(20026, [model.runtimeType, _modelName]);
  ...
  static Future del(User model) {
    if (model is! User) throw new IStoreException(20030, [model.runtimeType, _modelName]);
  ...
  static String _makeKey(User model) {
    var pk = model.getPK();
    if (pk is List) {
      if (pk.contains(null)) throw new IStoreException(20042, ['User']);
      return '${abb}:${pk.join(':')}';
    } else {
      if (pk == null) throw new IStoreException(20042, ['User}']);
      return '${abb}:${pk}';
    }
  }
