...
class UserSingleListMariaDBStore extends IMariaDBStore {
  ...
  static Future<UserSingleList> set(UserSingleList list) {
    if (list is! UserSingleList) throw new IStoreException(21039, [list.runtimeType, _listName]);
  ...
  static Future<UserSingleList> get(id) {
    ...
    return handler.prepareExecute(
      ...
      results.listen((row) {
        list.rawSet(new UserSingle()..fromList(row.toList()));
      }, onDone: () {
        c.complete(list);
      });
  ...
  static Future _addChildren(Map toAddList, ConnectionPool handler) {
    List params = [];
    toAddList.forEach((String childId, UserSingle model) {
    ...
    return handler
    .prepare(IMariaDBSQLPrepare.makeAdd(table, UserSingle._mapFull, UserSingle._columns))
  ...
  static Future _setChildren(Map toSetList, ConnectionPool handler) {
    ...
    toSetList.forEach((String childId, UserSingle model) {
  ...
  static Future _delChildren(Map toDelList, ConnectionPool handler) {
    ...
    toDelList.forEach((String childId, UserSingle model) {
      waitList.add(
          handler.prepareExecute(
