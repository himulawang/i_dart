...
class UserListMariaDBStore extends IMariaDBStore {
  ...
  static Future<UserList> set(UserList list) {
    if (list is! UserList) throw new IStoreException(21039, [list.runtimeType, _listName]);
  ...
  static Future<UserList> get(id) {
    ...
    return handler.prepareExecute(
      ...
      results.listen((row) {
        list.rawSet(new User()..fromList(row.toList()));
      }, onDone: () {
        c.complete(list);
      });
  ...
  static Future _addChildren(Map toAddList, ConnectionPool handler) {
    List params = [];
    toAddList.forEach((String childId, User model) {
    ...
    return handler
    .prepare(IMariaDBSQLPrepare.makeAdd(table, User._mapFull, User._columns))
  ...
  static Future _setChildren(Map toSetList, ConnectionPool handler) {
    ...
    toSetList.forEach((String childId, User model) {
  ...
  static Future _delChildren(Map toDelList, ConnectionPool handler) {
    ...
    toDelList.forEach((String childId, User model) {
      waitList.add(
          handler.prepareExecute(
