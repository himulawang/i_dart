...
class RoomRedisStore extends IRedisStore {
  static const String abb = 'r';
  static const Map store = const {"type":"redis","readWriteSeparate":false,"sharding":true,"shardMethod":"CRC32","master":"GameCache","slave":"GameCacheSlave","expire":86400,"mode":"Atom","abb":"r"};
  static const num expire = 86400;
  static const String _modelName = 'Room';

  static Future add(Room model) {
    if (model is! Room) throw new IStoreException(20022, [model.runtimeType, _modelName]);
  ...
  static Future set(Room model) {
    if (model is! Room) throw new IStoreException(20026, [model.runtimeType, _modelName]);
  ...
  static Future del(Room model) {
    if (model is! Room) throw new IStoreException(20030, [model.runtimeType, _modelName]);
  ...
  static String _makeKey(Room model) {
    var pk = model.getPK();
    if (pk is List) {
      if (pk.contains(null)) throw new IStoreException(20042, ['Room']);
      return '${abb}:${pk.join(':')}';
    } else {
      if (pk == null) throw new IStoreException(20042, ['Room}']);
      return '${abb}:${pk}';
    }
  }
