...
class UserSingleToAddLengthZeroIndexedDBStore extends IIndexedDBStore {
  static const Map store = const {"type":"indexedDB","master":"GameIDB","objectStore":"UserSingleToAddLengthZero"};
  static const String _modelName = 'UserSingleToAddLengthZero';

  static Future add(UserSingleToAddLengthZero model) {
    if (model is! UserSingleToAddLengthZero) throw new IStoreException(30004, [model.runtimeType, _modelName]);
  ...
  static Future set(UserSingleToAddLengthZero model) {
    if (model is! UserSingleToAddLengthZero) throw new IStoreException(30008, [model.runtimeType, _modelName]);
    ...
    Map toSetAbb = {};
    UserSingleToAddLengthZero._mapAbb.forEach((abb, i) {
      if (UserSingleToAddLengthZero._columns[i]['toSet']) return;
  ...
  static Future get(id) {
    UserSingleToAddLengthZero model = new UserSingleToAddLengthZero()..setPK(id);
  ...
  static Future del(UserSingleToAddLengthZero model) {
    if (model is! UserSingleToAddLengthZero) throw new IStoreException(30010, [model.runtimeType, _modelName]);
  ...
  static _makeKey(UserSingleToAddLengthZero model) {
    var pk = model.getPK();
    if (pk is List) {
